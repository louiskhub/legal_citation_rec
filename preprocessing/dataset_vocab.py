...
usc_re = re.compile(
    r"^(?P<chapter>\d+)\s+U[\.,]?\s?S[\.,]?\s?C[\.,]?\s?(A[\.,]?\s?)?\s*(\xa7+ *)?(?P<tail>.+)$"
)
cfr_re = re.compile(
    r"^(?P<chapter>\d+)\s+C[\.,]?\s?F[\.,]?\s?R[\.,]?\s*(\xa7+\s*)?(?P<tail>.+)$"
)
...
class CitationVocabulary:
    ...
    def init_empty_vocabulary(self) -> OrderedDict:
        cc = OrderedDict({NO_CITATION_TOKEN: 0, UNKNOWN_CITATION_TOKEN: 0})
        return cc
    ...
    def code_re_citation_stats(self):
        ...
        total_count = 0
        for cit, count in self.citation_counts.items():
            if (not cit.startswith(NORMAL_CODE_PREFIX)) and usc_re.search(
                cit
            ) is not None:
                num += 1
                total_count += count
    ...
    def reg_re_citation_stats(self):
        ...
        total_count = 0
        for cit, count in self.citation_counts.items():
            if (not cit.startswith(NORMAL_REG_PREFIX)) and cfr_re.search(
                cit
            ) is not None:
                num += 1
                total_count += count
    ...
    def presumed_case_citation_stats(self) -> Tuple[int, int]:
        ...
        case_count = (
            self.total_count() - code_total_count - reg_total_count - ncase_count
        )
    ...
    def load_from_case_dir(
        self, case_dir: str, case_ids: List[str] = None, case_id_fpath: str = None
    ):
        ...
        fnames = [fn for fn in os.listdir(case_dir) if fn.lower().endswith(".json")]
        if case_id_fpath is not None:
            case_ids = load_case_ids_from_file(case_id_fpath)
...
class CLACitationProcessor:
    ...
    def __init__(self):
        ...
        self.cla_ids = set()
        self.cit_regexes = (
            [  # reporter-specific regexes to extract volume and first page elements
    ...
    def add_cla_citation(self, cla_c: dict):
        ...
        official_cits = [
            cit["cite"] for cit in cla_c["citations"] if cit["type"] == "official"
        ]
        assert (
            len(official_cits) == 1
        ), f'case {cla_c["id"]} has no single official citation'
        oc = official_cits[0]
...
class CitationPredictionDataset(Dataset):
    def __init__(
        ...
        if case_ids_fpath is not None:
            case_ids = load_case_ids_from_file(case_ids_fpath)
        if case_ids is not None:
            fnames = [fn for fn in fnames if fn[:-5] in case_ids]
    ...
    def __is_encoded_citation_token(
        self, t_idx: int, ignore_unknown: bool = False
    ) -> bool:
    ...
    def __getitem__(self, idx: int):
        ...
        if self.tokenizer_mode == TOKENIZER_SENTENCE_PIECE:
            ...
            txt, cit_indices, case_meta = self.get_processed_case_text(
                idx, replace_citations=False
            )
        ...
        if self.pre_padding:
            ...
            pre_pad_length = max(
                0, self.context_length + self.forecast_length - len(encoded)
            )
        ...
        for i in range(self.context_length, len(padded)):
            if self.__is_encoded_citation_token(
                padded[i].item(), ignore_unknown=self.ignore_unknown
            ):
                new_pos = range(
                    i - self.context_length - self.forecast_length + 1,
        ...
        cit_tokens = [
            ...
            for fc in forecast
            if self.__is_encoded_citation_token(
                fc.item(), ignore_unknown=self.ignore_unknown
            )
        ]
        ...
        if self.return_type == "features":
            if self.add_case_meta:
                metas = torch.tensor(
                    [case_meta["year"], case_meta["issarea"], case_meta["judge"]]
                )
                attention_padding = torch.tensor([0] * len(metas))
                input = {
                    "input_ids": torch.cat([context, metas]),
    ...
    def train_sp_encoder(self, vocab_size=24000, model_prefix="bva_enc"):
        ...
        case_txt_iter = iter(
            [self.get_processed_case_text(i)[0] for i in range(len(self))]
        )
        citation_tokens = [f"@cit{i}@" for i in range(len(self.cit_vocabulary))]
        spm.SentencePieceTrainer.train(
            sentence_iterator=case_txt_iter,
